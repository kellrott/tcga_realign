...
def getPids(host):
	try:
		...
		for line in out.split("\n"):
			tmp = line.split(":")
			if len(tmp) == 2:
				name = os.path.basename(tmp[0]).replace(".pid", "").replace("tcga_realign_", "")
				pids[name] = tmp[1]
		return pids
...
if __name__ == "__main__":
	...
	for m in machines:
		...
		for i in input_list:
			job_state[i] = dict(job_state.get(i,{}), downloaded=True)
			
		for i in split_list:
			job_state[i] = dict(job_state.get(i,{}), split=True)
	...
	for i in job_location:
		for m in job_location[i]:
			if args.c is not None:
				...
				print i, m, job_location[i][m], ",".join(job_state.get(i, {'NA' : None}).keys())
